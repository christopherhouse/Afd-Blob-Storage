...
variable "enable_front_door_health_probe" {
  type        = bool
  description = "When true, configures the AFD origin group health probe to HEAD /health/health.txt. When false, the health probe is disabled entirely for the origin group. AFD does not support MI auth over Private Link, so health probes rely on anonymous blob access when enabled."
  default     = true
}
